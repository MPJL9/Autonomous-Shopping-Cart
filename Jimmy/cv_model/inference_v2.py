"""
Real-time inference for V2 model.

Usage:
    python inference_v2.py --dist_model distance_model_v2.npz --source 0
    python inference_v2.py --dist_model distance_model_v2.npz \
        --turn_model turn_model_v2.npz --source video.mp4
"""
...
def main():
    ...
    ap.add_argument("--dist_model", required=True)
    ap.add_argument("--turn_model", default=None)
    ...
    dist_model = DistanceMLP.load(args.dist_model)

    turn_model = None
    feat_buffer = None
    if args.turn_model:
        turn_model = TurnClassifierV2.load(args.turn_model)
    ...
    while True:
        ...
        if results.boxes is None or len(results.boxes) == 0:
            cv2.imshow("Follow-Me Cart V2", frame)
